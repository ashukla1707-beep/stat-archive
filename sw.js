...
const APP_SHELL = [
  ...
  "./assets/js/feature-polish.js","./assets/js/hero-layout-fix.js","./assets/js/phone-landscape-hero.js","./assets/js/action-spacing-fix.js","./assets/js/mobile-card-actions.js",
...
const PHONE_LANDSCAPE_HERO_TAG = '<script src="./assets/js/phone-landscape-hero.js?v=20260926-android-version-v1521"></script>';
...
const MOBILE_CARD_ACTIONS_TAG = '<script src="./assets/js/mobile-card-actions.js?v=20260926-mobile-toolbar-fit-v1"></script>';
...
function decorateNavigationHtml(html) {
  ...
  if (!out.includes('id="statTouchDesktopFirstPaintGuard"')) out = out.replace('</head>','<style id="statTouchDesktopFirstPaintGuard">@media (pointer:coarse) and (min-width:701px){.hero-probability>.probability-svg,.hero-probability>.axis-mid{visibility:hidden!important;opacity:0!important}.hero-probability>#statTouchDesktopGraph{visibility:visible!important;opacity:1!important}}</style>\n</head>');
  ...
  out = out.replace(/<script[^>]+assets\/js\/action-spacing-fix\.js[^>]*><\/script>/gi, '');
  out = out.replace(/<script[^>]+assets\/js\/mobile-card-actions\.js[^>]*><\/script>/gi, '');
  out = out.replace(/<script[^>]+assets\/js\/download-fix\.js[^>]*><\/script>/gi, '');
  ...
  if (out.includes(pdfLibTag)) out = out.replace(pdfLibTag, `${SPEED_SCRIPT_TAG}\n<script src="assets/js/runtime.js"></script>\n<script defer src="https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js"></script>`);
  else if (!out.includes('assets/js/speed-boost.js')) out = out.replace('</body>', `${SPEED_SCRIPT_TAG}\n<script src="assets/js/runtime.js"></script>\n</body>`);
  ...
  out = out.replace('<script src="assets/js/service-worker-register.js"></script>',`${STARTUP_POLISH_TAG}\n<script src="assets/js/service-worker-register.js"></script>`);
  ...
  /* Canonical card-action order: base/mobile layout first, Download runtime second, final Download=Offline visual sync last. */
  out = out.replace('</body>', `${MOBILE_CARD_ACTIONS_TAG}\n${ACTION_SPACING_FIX_TAG}\n</body>`);
  return out;
}

function isAppShellNavigation(url) { const path = url.pathname || "/"; return path.endsWith("/") || path.endsWith("/index.html"); }
async function normalizeSameOriginResponse(response, url, isAppNavigation) {
  if (!response || !response.ok) return response;
  if (isAppNavigation) { const html = await response.text(); return new Response(decorateNavigationHtml(html), {status:response.status,statusText:response.statusText,headers:response.headers}); }
  if (url.pathname.endsWith('/assets/scanner.css')) { const css = await response.text(); return new Response(css + MENU_FLASH_GUARD, {status:response.status,statusText:response.statusText,headers:response.headers}); }
  return response;
}
self.addEventListener('install', event => { event.waitUntil((async () => { const cache=await caches.open(CACHE); await Promise.allSettled(APP_SHELL.map(async asset=>{try{const request=new Request(asset,{cache:'reload'});const response=await fetch(request);if(!response||!response.ok)return;const url=new URL(request.url);const isAppNavigation=asset==='./'||asset==='./index.html';const finalResponse=await normalizeSameOriginResponse(response,url,isAppNavigation);await cache.put(request,finalResponse.clone());}catch(_){}}));await self.skipWaiting();})()); });
self.addEventListener('activate', event => { event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key!==CACHE&&key!==EXTERNAL_CACHE).map(key=>caches.delete(key)));await self.clients.claim();})()); });
async function updateSameOriginInBackground(request,url,isAppNavigation){try{const response=await fetch(new Request(request,{cache:'no-store'}));if(!response||!response.ok)return;const finalResponse=await normalizeSameOriginResponse(response,url,isAppNavigation);const cache=await caches.open(CACHE);await cache.put(request,finalResponse.clone());}catch(_){}}
async function serveAppShellFast(request,url,isAppNavigation,event){const cache=await caches.open(CACHE);let cached=await cache.match(request);if(!cached&&isAppNavigation)cached=(await cache.match('./index.html'))||(await cache.match('./'));if(cached){event.waitUntil(updateSameOriginInBackground(request,url,isAppNavigation));return cached;}try{const response=await fetch(new Request(request,{cache:'no-store'}));if(!response||!response.ok)return response;const finalResponse=await normalizeSameOriginResponse(response,url,isAppNavigation);cache.put(request,finalResponse.clone()).catch(()=>{});return finalResponse;}catch(_){return Response.error();}}
async function serveRuntimeNetworkFirst(request){const cache=await caches.open(CACHE);try{const freshRequest=new Request(request,{cache:'no-store'});const response=await fetch(freshRequest);if(response&&response.ok){cache.put(request,response.clone()).catch(()=>{});return response;}return(await cache.match(request))||response;}catch(_){return(await cache.match(request))||Response.error();}}
async function fetchExternalFast(request,event){const cache=await caches.open(EXTERNAL_CACHE);const cached=await cache.match(request);if(cached){event.waitUntil(fetch(request).then(response=>{if(response&&(response.ok||response.type==='opaque'))return cache.put(request,response.clone());}).catch(()=>{}));return cached;}const response=await fetch(request);if(response&&(response.ok||response.type==='opaque'))cache.put(request,response.clone()).catch(()=>{});return response;}
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin===self.location.origin&&url.pathname.startsWith('/api/reviews')){event.respondWith(fetch(new Request(request,{cache:'no-store'})));return;}if(url.origin!==self.location.origin){const cacheableExternal=url.hostname==='cdn.jsdelivr.net'||url.hostname==='cdnjs.cloudflare.com'||url.hostname==='fonts.googleapis.com'||url.hostname==='fonts.gstatic.com';event.respondWith(cacheableExternal?fetchExternalFast(request,event):fetch(request));return;}if(url.origin===self.location.origin&&url.pathname.endsWith('/version.json')){event.respondWith(fetch(new Request(request,{cache:'no-store'})));return;}const isDocumentNavigation=request.mode==='navigate';const isAppNavigation=isDocumentNavigation&&isAppShellNavigation(url);if(isAppNavigation){event.respondWith(serveAppShellFast(request,url,true,event));return;}if(isDocumentNavigation){event.respondWith(serveAppShellFast(request,url,false,event));return;}const isLiveRuntime=url.pathname.endsWith('/assets/js/preview.js')||url.pathname.endsWith('/assets/js/pdf.js')||url.pathname.endsWith('/assets/js/preview-state-guard.js')||url.pathname.endsWith('/assets/js/download-fix.js')||url.pathname.endsWith('/assets/js/download-progress-canonical.js')||url.pathname.endsWith('/assets/js/speed-boost.js')||url.pathname.endsWith('/assets/js/search-suggestions.js')||url.pathname.endsWith('/assets/js/feature-polish.js')||url.pathname.endsWith('/assets/js/startup-polish.js')||url.pathname.endsWith('/assets/js/hero-animation.js')||url.pathname.endsWith('/assets/js/hero-layout-fix.js')||url.pathname.endsWith('/assets/js/phone-landscape-hero.js')||url.pathname.endsWith('/assets/js/action-spacing-fix.js')||url.pathname.endsWith('/assets/js/mobile-card-actions.js')||url.pathname.endsWith('/assets/js/tooltips.js')||url.pathname.endsWith('/assets/js/service-worker-register.js')||url.pathname.endsWith('/assets/js/accessibility.js')||url.pathname.endsWith('/assets/js/menu-polish.js')||url.pathname.endsWith('/assets/js/public-reviews.js')||url.pathname.endsWith('/assets/js/menu-alignment-fix.js')||url.pathname.endsWith('/assets/js/menu-header-reference.js')||url.pathname.endsWith('/assets/js/offline-library-hybrid.js')||url.pathname.endsWith('/assets/js/scroll-lock-coordinator.js')||url.pathname.endsWith('/assets/js/offline-library-handoff.js');if(isLiveRuntime){event.respondWith(serveRuntimeNetworkFirst(request));return;}const isMutableAppAsset=url.pathname.includes('/assets/js/')||url.pathname.endsWith('/assets/styles.css')||url.pathname.endsWith('/assets/scanner.css')||url.pathname.endsWith('/manifest.json');if(isMutableAppAsset){event.respondWith(serveAppShellFast(request,url,false,event));return;}event.respondWith((async()=>{const cache=await caches.open(CACHE);const cached=await cache.match(request);if(cached)return cached;const response=await fetch(request);if(response&&response.ok)cache.put(request,response.clone()).catch(()=>{});return response;})());});