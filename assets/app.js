...
async function refreshArchiveSilently() {
  ...
  try {
    const [freshEntries] = await Promise.all([loadEntries(), loadSubjectsFromWorker()]);
    const oldSignature = entries.map(e => `${e.id}:${e.uploadedAt}:${e.size}`).join("|");
    const newSignature = freshEntries.map(e => `${e.id}:${e.uploadedAt}:${e.size}`).join("|");
