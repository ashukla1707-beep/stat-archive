...
(function setupEditDriveToggle(){
  ...
  function syncForType() {
    ...
    if (!allowed && editDriveLinkMode) {
      ...
      showEditEntryError(
        "Google Drive links are only available for Notes, Book, and Others. Choose a replacement file for this type."
      );
    } else {
      onBtn.style.display =
        (!editDriveLinkMode && allowed)
          ? "inline-flex"
          : "none";
...
function populateEditSubjectOptions(selectedCode) {
  ...
  const sel =
    document.getElementById("editSubjectSelect");
...
function updateEditEntryFields() {
  ...
  if (
    YEAR_VISIBLE_TYPES.includes(type)
  ) {
  ...
  if (type === "Notes") {
    ...
    titleLabel.textContent =
      "Title (Preserve the 'Subject — Notes:' prefix)";

    titleInput.placeholder =
      "e.g., Probability Theory — Notes: Module 1";
  }
  ...
  if (
    ...
    fileInput.files[0].size >
      maxBytesForType(type)
  ) {

    showEditEntryError(
      `The selected replacement file is too large for ${type}. Choose a smaller file or change the type back.`
    );
  }
}
...
function canEditEntry(entry) {
  ...
  if (
    entry.contributorEditable === true
  ) {
    return true;
  }


  return getContributorLatest3Ids()
    .has(
      String(entry.id)
    );
}
...
function openEditEntry(entry) {
  ...
  if (
    ...
    showError(
      "Contributors can edit only the 3 newest entries. Older entries require Admin permission."
    );
  ...
  .addEventListener(
    ...
    () => {
      ...
      if (
        ...
        (
          input.value.trim() ===
            editingOriginalTitle.trim() ||

          input.value.trim() ===
            `${oldSubjectName} — ${canonicalEntryType(
              entries.find(
                item =>
                  String(item.id) ===
    ...
    async e => {
      ...
      try {
        ...
        if (!entry) {
          ...
          showEditEntryError(
            "Entry not found. Refresh and try again."
          );
        ...
        if (
          ...
          try {
            ...
            selectedSubjectMeta.created_by =
              createdSubject.created_by ||
              selectedSubjectMeta.created_by;
            ...
            showEditEntryError(
              err?.message ||
              "Could not prepare that subject for editing."
            );
        ...
        if (
          ...
          if (
            ...
            showEditEntryError(
              "That doesn't look like a Google Drive link. Copy the \"Share\" link from Drive."
            );
          ...
          if (
            ...
            showEditEntryError(
              "Google Drive links are only available for Notes, Book, and Others."
            );
        ...
        if (
          ...
          showEditEntryError(
            "This entry currently uses Google Drive. Keep the Drive link or choose a replacement file."
          );
        ...
        if (
          ...
          showEditEntryError(
            type === "Book"
              ...
              : "Add a title describing the file."
          );
        ...
        if (
          ...
          showEditEntryError(
            "Enter a valid 4-digit year, or leave the year blank."
          );
        ...
        if (
          ...
          showEditEntryError(
            `A ${type} for ${year} already exists for this subject.`
          );
        ...
        if (file) {
          ...
          if (
            ...
            showEditEntryError(
              `That file is ${formatSize(file.size)}. ${type} files must be under ${formatSize(maxBytesForType(type))}.`
            );
        ...
        try {
          ...
          pendingCarouselPositions =
            captureCarouselPositions();
...
async function deleteEntry(
  ...
  if (!allowedByUi) {
    ...
    showError(
      "Only an Admin can delete archive entries."
    );
...
const YEAR_REQUIRED_TYPES = [
  "Previous Year Question",
  "Mid-Term Question"
];


const YEAR_OPTIONAL_TYPES = [
  "Notes",
  "Others"
];


const YEAR_VISIBLE_TYPES = [
  ...YEAR_REQUIRED_TYPES,
  ...YEAR_OPTIONAL_TYPES
];
...
function updateFileSizeHint() {
  ...
  if (
    ...
    if (
      ...
      showFormError(
        `Current file is too large (${formatSize(file.size)}). Limit for ${type} is ${formatSize(typeLimit)}.`
      );
...
window.statArchiveNativePickerActive =
  false;


const archiveFileInput =
  document.getElementById(
    "fileInput"
  );
  ...
  .addEventListener(
    ...
    e => {
      ...
      if (
        ...
        showFormError(
          `File is too large (${formatSize(file.size)}). Limit for ${selectedType} is ${formatSize(typeLimit)}.`
        );
...
function getCurrentCredentialRole() {
    ...
    async () => {
      ...
      if (session) {
        ...
        try {
          ...
          console.warn(
            "Network failed during manual sign out.",
            err
          );
...
function hasDuplicateQuestionEntry(
  subjectCode,
  type,
  year,
  excludeEntryId = null
    ...
    async e => {
      ...
      if (
        ...
        if (!file) {
          ...
          showFormError(
            "Choose a file to file into the archive."
          );
      ...
      if (
        ...
        try {
          ...
          showFormError(
            err?.message ||
            "Could not prepare that subject for filing."
          );
      ...
      if (
        ...
        showFormError(
          `A ${selectedType} for ${yearVal} already exists for this subject.`
        );
